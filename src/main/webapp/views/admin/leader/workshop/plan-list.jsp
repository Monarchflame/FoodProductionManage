            ...
            <ul class="nav">
                <li>
                    ...
                    <ul class="menu-collapse collapse in" id="ui_menu_workshop">
                        <li>
                            <a href="${pageContext.request.contextPath}/admin/leader/workshop/leader/plan-list"><i class="icon icon-lg">account_box</i>&nbsp;查看生产计划</a>
                        </li>
                        <li>
                            <a href="${pageContext.request.contextPath}/admin/leader/workshop/leader/staff-list"><i class="icon icon-lg">account_box</i>&nbsp;查看员工</a>
                        </li>
                    </ul>
            </ul>
        </div>
...
<script>
    $(document).ready(function(){
        $.ajax({
            type: "POST",
            url: "${pageContext.request.contextPath}/admin/leader/workshop/leader/planInfoList",
            dataType: "json",
            data: {
            },
            //查询生产计划信息返回相应数据
            success: function(data) {
                ...
                var readyProductionPlan = [];
                var inProductionPlanList = [];
                ...
                for (let i = 0; i < planList.length; i++) {
                    if (planList[i].status === "未执行") {
                        readyProductionPlan.push(planList[i])
                    } else if (planList[i].status === "执行中") {
                        inProductionPlanList.push(planList[i])
                    }
                }
                //分割
                var readyProductionPlanHTML = [];
                var inProductionPlanHTML = [];
                //未执行计划
                for(let i = 0;i < readyProductionPlan.length;i++) {
                    let plan = readyProductionPlan[i];
                    readyProductionPlanHTML.push('<div class="node-card node-flex" onclick="productionPlan(' + plan.plan_id + ')">' +
                        '<div class="nodemain">' +
                ...
                $('#plangroup0').html(readyProductionPlanHTML.join(''));
        ...
        productionPlan = function (id) {
            document.getElementById('infoifram').src = '${pageContext.request.contextPath}/admin/leader/workshop/leader/productionPlanInfo?planId=' + id;
            $("#planinfo").modal();
        };

        //点击执行中的计划
        inProductionPlan = function (id) {
            document.getElementById('infoifram').src = '${pageContext.request.contextPath}/admin/leader/workshop/leader/inProductionPlanInfo?planId=' + id;
            $("#planinfo").modal();
        };
    })
</script>
